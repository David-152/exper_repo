...
![Концептуальная модель](./documents/concept-model.png)

#### Логическая модель
![Логическая модель](./documents/logical-model.png)
...
В базе данных реализовано **SCD Type 2** в следующих таблицах:
1. **Сотрудники (Employees)**:
   - Поля `start_date`, `end_date` и `is_current` используются для хранения истории изменений данных о сотрудниках.
   - При изменении данных создаётся новая запись, а старая помечается как неактуальная (`is_current = FALSE`).

2. **Материалы (Materials)**:
   - Поля `start_date`, `end_date` и `is_current` используются для хранения истории изменений данных о материалах.
   - При изменении данных создаётся новая запись, а старая помечается как неактуальная (`is_current = FALSE`).
